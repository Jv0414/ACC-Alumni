...
const AlumniHome = ({ onCheckStatus }: AlumniHomeProps) => {
  return (
    <div className="alumni-desktop-page">
      ...
      {onCheckStatus && (
        <section className="alumni-card alumni-status-cta">
          <div className="alumni-status-cta-text">
            <h3>
              <FileSearch size={16} /> Already registered?
            </h3>
            <p>
              Enter the reference number you received when you submitted your registration to
              see whether it is still pending and review the information you submitted.
            </p>
          </div>
          <button className="btn btn-primary alumni-status-cta-btn" onClick={onCheckStatus}>
            <FileSearch size={16} />
